...
export default {
  ...
  mounted () {
    ...
    document.getElementById('cardModal').addEventListener('show.bs.modal', () => {
      setTimeout(() => {
        autosize.update(document.getElementsByClassName('auto-size'))
      }, 0)
  ...
  methods: {
    changeCardTitle () {
      cardService.changeCardTitle(this.cardId, this.title).then(() => {
        ...
        document.getElementById('cardModal').focus()
      }).catch(error => {
