...
public class DefaultMailManager implements MailManager {
    private final Logger logger = LoggerFactory.getLogger(DefaultMailManager.class);
    ...
    public void send(String email, String subject, String template, MessageVariable... variables) {
        Assert.hasText(email, "Parameter `email` must not be blank");
        ...
        Message message = new SimpleMessage(email, subject, messageBody, mailFrom);
        mailer.send(message);
    }
    ...
    private String createMessageBody(String templateName, MessageVariable... variables) {
        try {
            ...
            logger.error("Failed to create message body from template `" + templateName + "`", e);
            return null;
        }
    }
}
