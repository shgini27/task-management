...
import org.ttweb.taskmanagement.TaskManagementApplication;
...
@SpringBootTest(webEnvironment = SpringBootTest.WebEnvironment.MOCK, classes = TaskManagementApplication.class)
@ActiveProfiles("test")
...
public class RegistrationApiIntegrationTests {
    ...
    public void register_existedUsername_shouldFailAndReturn400() throws Exception {
        ...
        mvcMock.perform(
                post("/api/registrations")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(Objects.requireNonNull(JsonUtils.toJson(payload2))))
                .andExpect(status().is(400))
